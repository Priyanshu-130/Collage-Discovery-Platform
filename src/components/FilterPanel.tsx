...
export default function FilterPanel({ filters, onChange, onReset }: FilterPanelProps) {
  // Extract unique cities dynamically from mock database
  const cities = Array.from(new Set(colleges.map((c) => c.city))).sort();

  // Extract unique exams dynamically from mock database
  const entranceExams = Array.from(
    new Set(colleges.flatMap((c) => c.acceptedExams || []))
  ).sort();
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <select
            ...
            className="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3 text-sm font-semibold text-slate-700 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
          >
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => handleFilterChange('minNirf', parseInt(e.target.value))}
            className="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3 text-sm font-semibold text-slate-700 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
          >
            {nirfOptions.map((opt) => (
        ...
        <div>
          ...
          <input
            ...
            className="h-1.5 w-full cursor-pointer rounded-lg bg-slate-200 accent-indigo-600 appearance-none focus:outline-none"
          />
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() => handleFilterChange('sortBy', sortOption.value as 'rating' | 'fees' | 'placement')}
                  className={`rounded-xl border py-2 text-xs font-bold transition-all ${
