...
export default function CompareTable({ collegesToCompare }: CompareTableProps) {
  ...
  if (collegesToCompare.length === 0) return null;
  ...
  const renderRow = (label: string, accessor: (c: College) => React.ReactNode, highlightBetter?: boolean, betterValueSelector?: (c: College) => number) => {
    // If highlightBetter is active, determine which index is superior
    let betterIndex = -1;
    if (highlightBetter && collegesToCompare.length === 2 && betterValueSelector) {
      ...
      if (val0 > val1) betterIndex = 0;
      else if (val1 > val0) betterIndex = 1;
    }
      ...
      <tr className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
        ...
        {collegesToCompare.map((college, idx) => {
          ...
          return (
            <td 
              key={college.id} 
              className={`py-4 px-6 text-sm font-semibold text-slate-700 ${
                isWinner ? 'bg-indigo-50/30 text-slate-800' : ''
              }`}
            >
              <div className="flex items-center gap-1.5">
                {accessor(college)}
                {isWinner && (
                  <span title="Higher Metric">
        ...
        {/* If only one college is selected, render a placeholder column */}
        {collegesToCompare.length === 1 && (
          <td className="py-4 px-6 text-sm text-slate-400 italic bg-slate-50/50">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50/30">
              ...
              {collegesToCompare.map((college) => (
                <th key={college.id} className="py-5 px-6 relative min-w-[280px]">
                  {/* Remove Button */}
                  <button
                    onClick={() => removeFromCompare(college.id)}
                    className="absolute top-4 right-4 flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 hover:text-slate-600 hover:border-slate-300 transition-all shadow-sm"
              ...
              {/* Empty placeholder column if comparing only 1 */}
              {collegesToCompare.length === 1 && (
                <th className="py-5 px-6 text-center bg-slate-50/50 min-w-[280px]">
          ...
          <tbody>
            ...
            {renderRow(
              'Rating',
              (c) => <RatingBadge rating={c.rating} reviewsCount={c.reviewsCount} />,
              true,
              (c) => c.rating
            )}
            ...
            {renderRow(
              'NIRF Ranking',
              (c) => c.nirfRank ? <span className="font-bold text-slate-800">#{c.nirfRank}</span> : <span className="text-slate-400">N/A</span>,
              true,
              (c) => c.nirfRank ? -c.nirfRank : -9999
            )}

            {/* Sub-Ratings */}
            {renderRow(
              'Academics Rating',
              (c) => <span className="font-semibold text-slate-700">{c.subRatings.academics.toFixed(1)} / 5.0</span>,
              true,
              (c) => c.subRatings.academics
            )}
            {renderRow(
              'Placements Rating',
              (c) => <span className="font-semibold text-slate-700">{c.subRatings.placements.toFixed(1)} / 5.0</span>,
              true,
              (c) => c.subRatings.placements
            )}
            {renderRow(
              'Infrastructure Rating',
              (c) => <span className="font-semibold text-slate-700">{c.subRatings.infra.toFixed(1)} / 5.0</span>,
              true,
              (c) => c.subRatings.infra
            )}
            {renderRow(
              'Campus Life Rating',
              (c) => <span className="font-semibold text-slate-700">{c.subRatings.campusLife.toFixed(1)} / 5.0</span>,
              true,
              (c) => c.subRatings.campusLife
            )}
            ...
            {renderRow(
              ...
              (c) => <span className="font-extrabold text-slate-800">{formatLPA(c.placement.averagePackage)}</span>,
              true,
              (c) => c.placement.averagePackage
            )}

            {/* Highest Package */}
            {renderRow(
              'Highest Salary Package',
              (c) => <span className="font-extrabold text-slate-800">{formatLPA(c.placement.highestPackage)}</span>,
              true,
              (c) => c.placement.highestPackage
            )}
                ...
                <div className="flex w-full items-center gap-2">
                  <div className="h-2 w-24 overflow-hidden rounded-full bg-slate-100 border border-slate-200">
                    <div 
                      ...
                      style={{ width: `${c.placement.placementRate}%` }} 
                    />
                  </div>
            ...
            {renderRow('Key Facilities', (c) => (
              <div className="flex flex-wrap gap-1">
                {c.facilities.slice(0, 5).map((fac) => (
                  <span key={fac} className="rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">
                    {fac}
                  </span>
                ))}
                {c.facilities.length > 5 && (
                  <span className="text-[10px] font-bold text-slate-400 align-middle px-1">
                    +{c.facilities.length - 5}
                  </span>
                )}
              </div>
            ...
            {/* Courses summary */}
            {renderRow('Popular Courses Offered', (c) => (
              <div className="space-y-1 max-w-[240px]">
                {c.courses.map((course) => (
                  <div key={course.name} className="flex justify-between items-center text-xs border-b border-slate-100 pb-1">
            ...
            {/* Recruiters */}
            {renderRow('Top Recruiters', (c) => (
              <div className="flex flex-wrap gap-1.5 max-w-[240px]">
                {c.placement.topRecruiters.map((rec) => (
                  <span key={rec} className="rounded bg-slate-50 border border-slate-150 px-1.5 py-0.5 text-[10px] font-semibold text-slate-600">
                    {rec}
                  </span>
                ))}
              </div>
