...
export default function AuthModal({ isOpen, onClose, initialMode = 'login' }: AuthModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      // Login simulation - default user name to Email handle if not found
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm">
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl transition-all duration-300">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-10 pr-4 text-sm text-slate-800 placeholder-slate-400 shadow-inner outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all"
                />
              </div>
            </div>
          )}
          ...
          <button
            type="submit"
            className="w-full rounded-xl bg-slate-900 py-3.5 text-center text-sm font-bold text-white shadow-lg shadow-slate-900/10 hover:bg-indigo-600 hover:shadow-indigo-600/10 active:scale-[0.98] transition-all"
          >
