...
export default function CollegeCard({ college }: CollegeCardProps) {
    ...
    return (<div className="group flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
      
      <div className="relative h-48 w-full overflow-hidden bg-slate-100">
        
        <img src={college.image} alt={college.name} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy"/>
        
        <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 via-transparent to-transparent"/>
        
        
        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
          {college.nirfRank && (<span className="rounded-lg bg-amber-500 px-2.5 py-1 text-xs font-bold text-white shadow-sm">
        ...
        }} className={`absolute top-4 right-4 flex h-9 w-9 items-center justify-center rounded-xl backdrop-blur-md transition-all border shadow-sm ${saved
            ? 'bg-rose-500 border-rose-500 text-white'
            : 'bg-white/90 border-slate-200 text-slate-600 hover:bg-white hover:text-slate-900'}`} title={saved ? 'Remove Bookmark' : 'Save College'}>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {college.placement.topRecruiters.slice(0, 3).map((recruiter) => (<span key={recruiter} className="rounded-md bg-white border border-slate-200 px-2 py-0.5 text-[10px] font-semibold text-slate-600">
                {recruiter}
              </span>))}
            {college.placement.topRecruiters.length > 3 && (<span className="text-[10px] font-bold text-slate-400 self-center pl-0.5">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
          <button onClick={() => toggleCompareCollege(college.id)} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold border transition-all ${comparing
          ...
          <Link href={`/college/${college.id}`} className="flex items-center gap-1 text-xs font-bold text-indigo-600 hover:text-indigo-800 transition-colors">
            <span>Details</span>
