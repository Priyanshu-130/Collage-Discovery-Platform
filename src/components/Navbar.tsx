...
export default function Navbar({ onMenuClick }: NavbarProps) {
    ...
    return (<header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white/70 px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-3">
        ...
        {user ? (<div className="flex items-center gap-2 pl-1.5 border-l border-slate-200 ml-1.5">
            <button onClick={logout} title="Sign Out" className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-tr from-indigo-500 to-violet-500 font-bold text-white text-sm shadow-sm ring-2 ring-white hover:scale-105 active:scale-95 transition-all">
