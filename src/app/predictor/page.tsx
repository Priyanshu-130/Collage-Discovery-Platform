...
import { Sparkles, MapPin, BookOpen, GitCompare, Bookmark, Info, AlertCircle, CheckCircle, HelpCircle, Clock, ExternalLink, ChevronRight, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function PredictorPage() {
    ...
    const predictions = useMemo<PredictionResult[]>(() => {
        if (!hasPredicted || !rankInput || isNaN(Number(rankInput)))
            return [];
        ...
        colleges.forEach((col) => {
            const accepts = col.acceptedExams.some((ex) => ex.toLowerCase() === selectedExam.toLowerCase());
            if (accepts) {
    ...
    const handlePredictSubmit = (e: React.FormEvent) => {
        ...
        if (!rankInput || isNaN(rankNum) || rankNum <= 0)
            return;
        setHasPredicted(true);
        const matchCount = colleges.filter((col) => col.acceptedExams.some((ex) => ex.toLowerCase() === selectedExam.toLowerCase())).length;
        setHistory((prev) => {
    ...
    return (<div className="space-y-8 flex-1 flex flex-col justify-start">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-1">
            ...
            <form onSubmit={handlePredictSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-1 border border-slate-100 rounded-xl p-1 bg-slate-50 shadow-inner">
                  {EXAMS_LIST.map((exam) => (<button key={exam.id} type="button" onClick={() => {
                setSelectedExam(exam.id);
                setHasPredicted(false);
            }} className={`flex items-center justify-between rounded-lg p-2.5 text-left transition-all ${selectedExam === exam.id
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="number" min="1" max="999999" value={rankInput} onChange={(e) => {
            setRankInput(e.target.value);
            setHasPredicted(false);
        }} placeholder={EXAMS_LIST.find((e) => e.id === selectedExam)?.placeholder || 'e.g. 5000'} className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-14 pr-4 text-sm text-slate-800 placeholder-slate-400 shadow-inner outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all font-semibold" required/>
                </div>
              </div>

              <button type="submit" className="w-full rounded-xl bg-slate-900 py-3.5 text-center text-sm font-bold text-white shadow-lg hover:bg-indigo-600 active:scale-[0.98] transition-all">
          ...
          {history.length > 0 && (<div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
              <div className="flex items-center gap-2 text-slate-500">
                <Clock className="h-4.5 w-4.5"/>
                <h4 className="text-xs font-bold uppercase tracking-wider">Prediction Logs</h4>
              </div>
              <div className="space-y-2">
                {history.map((h, i) => (<button key={`history-${i}`} onClick={() => handleHistoryClick(h.exam, h.rank)} className="flex w-full items-center justify-between rounded-xl border border-slate-100 bg-slate-50/50 p-3 text-left hover:bg-slate-50 hover:border-slate-200 transition-all">
                    <div>
        ...
        <div className="space-y-6 lg:col-span-2">
          {hasPredicted ? (predictions.length > 0 ? (<div className="space-y-6">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                    {branchSummary.map((b) => {
                return (<div key={`branch-sum-${b.branchName}`} className="rounded-xl border border-slate-100 bg-slate-50/50 p-3.5 space-y-2.5 hover:border-indigo-100 hover:bg-slate-50 transition-all shadow-sm">
                          <span className="block text-xs font-bold text-slate-800">{b.branchName}</span>
                          <div className="flex flex-wrap gap-1.5">
                            {b.colleges.length > 0 ? (b.colleges.map((c) => {
                        const badgeStyles = {
                        ...
                        return (<button key={`branch-sum-${b.branchName}-${c.collegeId}`} onClick={() => {
                                const el = document.getElementById(`prediction-${c.collegeId}`);
                                if (el) {
                                    el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                    setExpandedColleges(prev => ({ ...prev, [c.collegeId]: true }));
                                }
                            }} className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-bold transition-all shadow-sm ${badgeStyles[c.chance]}`} title={`Click to view details. Chance: ${c.chance}`}>
                                    <span>{c.collegeName}</span>
                                    {c.chance !== 'Unlikely' ? (<span className="text-[8px] font-bold uppercase shrink-0">({c.chance[0]})</span>) : (<span className="text-[8px] font-bold uppercase shrink-0">(U)</span>)}
                                  </button>);
                ...
                return (<div key={`prediction-${col.id}`} id={`prediction-${col.id}`} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:border-indigo-100 hover:shadow-md transition-all group relative overflow-hidden">
                        
                        <div className={`absolute top-0 bottom-0 left-0 w-1 ${p.chance === 'High' ? 'bg-emerald-500' : p.chance === 'Medium' ? 'bg-amber-500' : p.chance === 'Low' ? 'bg-rose-500' : 'bg-slate-350'}`}/>
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="space-y-1">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {col.nirfRank && (<span className="rounded-md bg-slate-100 border border-slate-200 px-1.5 py-0.5 text-[10px] font-bold text-slate-600">
                                    NIRF #{col.nirfRank}
                                  </span>)}
                              </div>
                              ...
                              <div className="mt-3">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                  {eligibleBranches.length > 0 ? (eligibleBranches.slice(0, 3).map((br) => {
                        const miniBadgeStyles = {
                            High: 'bg-emerald-50 text-emerald-700 border-emerald-100',
                            Medium: 'bg-amber-50 text-amber-700 border-amber-100',
                            Low: 'bg-rose-50 text-rose-700 border-rose-100'
                        };
                        return (<span key={`mini-branch-${br.branchName}`} className={`inline-block rounded-lg border px-2 py-0.5 text-[10px] font-bold ${miniBadgeStyles[br.chance as 'High' | 'Medium' | 'Low']}`}>
                                  ...
                                  {eligibleBranches.length > 3 && (<span className="inline-block rounded-lg bg-slate-50 border border-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-500">
                          ...
                          <div className="flex flex-wrap gap-2 sm:flex-col sm:items-end justify-start">
                            <button onClick={() => router.push(`/college/${col.id}`)} className="inline-flex items-center gap-1 rounded-xl bg-slate-50 border border-slate-200 px-3.5 py-2 text-xs font-bold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-100 transition-all">
                              <span>View Profile</span>
                              <ExternalLink className="h-3.5 w-3.5"/>
                            </button>
                            <div className="flex gap-2">
                              <button onClick={() => toggleCompareCollege(col.id)} className={`flex h-8 w-8 items-center justify-center rounded-lg border transition-all ${isComparing(col.id)
                        ? 'bg-indigo-600 border-indigo-600 text-white shadow'
                        : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`} title={isComparing(col.id) ? "Comparing" : "Add to Compare"}>
                                <GitCompare className="h-4 w-4"/>
                              </button>
                              <button onClick={() => toggleSaveCollege(col.id)} className={`flex h-8 w-8 items-center justify-center rounded-lg border transition-all ${isSaved(col.id)
                        ? 'bg-red-500 border-red-500 text-white shadow'
                        : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`} title={isSaved(col.id) ? "Bookmarked" : "Bookmark"}>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between pl-2">
                          <button type="button" onClick={() => toggleExpandCollege(col.id)} className="inline-flex items-center gap-1.5 text-xs font-bold text-indigo-600 hover:text-indigo-800 transition-colors">
                            {isExpanded ? (<>
                        ...
                        {isExpanded && (<div className="mt-4 rounded-2xl border border-slate-150 overflow-hidden pl-2 shadow-inner bg-slate-50/30">
                            ...
                            return (<tr key={`tbl-branch-${br.branchName}`} className="hover:bg-slate-50/50 transition-colors">
              ...
              </div>) : (<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 py-16 px-6 text-center">
              ...
              </div>)) : (<div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 px-6 text-center shadow-sm">
