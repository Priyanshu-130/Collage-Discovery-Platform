...
import { GraduationCap, Briefcase, Star, MapPin, ArrowRight, Sparkles, History } from 'lucide-react';
...
export default function DashboardHome() {
    ...
    return (<div className="space-y-8 flex-1 flex flex-col justify-start">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <StatsCard title="Total Colleges" value={totalColleges} icon={GraduationCap} description="Accredited institutes" color="indigo"/>
        <StatsCard title="Average Placement" value={avgPlacement} icon={Briefcase} description="Salary packages offered" trend={{ value: "+8.2%", type: "positive" }} color="emerald"/>
        <StatsCard title="Top Rated Colleges" value={topRatedCount} icon={Star} description="Colleges rated 4.8+ Stars" color="amber"/>
        <StatsCard title="Cities Covered" value={uniqueCitiesCount} icon={MapPin} description="Academic hubs indexed" color="rose"/>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => router.push('/colleges')} className="inline-flex items-center gap-1 text-xs font-bold text-indigo-600 hover:text-indigo-800 transition-colors">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            {featuredColleges.slice(0, 2).map((college) => (<CollegeCard key={college.id} college={college}/>))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            {recentlyViewedColleges.length > 0 ? (recentlyViewedColleges.map((college) => (<div key={`recent-${college?.id}`} className="flex items-center gap-3.5 rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm hover:border-indigo-100 hover:shadow transition-all group cursor-pointer" onClick={() => router.push(`/college/${college?.id}`)}>
                  
                  <img src={college?.image} alt={college?.name} className="h-14 w-14 shrink-0 rounded-xl object-cover border border-slate-100"/>
                ...
                </div>))) : (<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-slate-400">
